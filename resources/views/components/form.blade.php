...
<div class="mx-2 text-xs rounded w-80 min-h-fit hover:bg-yellow-100 min-w-fit max-h-28 outline outline-1 outline-gray-300">
    <form class="flex flex-col items-center p-2 max-lg:p-2" name="{{ $formName }}" id="{{ $formId }}" method="{{ $method }}" action="{{ url($action) }}" >
        @csrf
        <div>
        ...
        <button class="px-2 py-1 mt-4 font-sans text-gray-600 bg-blue-100 rounded-md hover:text-blue-700 hover:font-bold hover:outline-2 hover:outline-blue-400 outline outline-1 outline-gray-100 hover:shadow-lg hover:bg-blue-300" type="{{ $buttonType }}" onclick="{{ $buttonOnclick }}" style="display: {{ $style }}" name="{{ $btnName }}" id="{{ $btnId }}">
