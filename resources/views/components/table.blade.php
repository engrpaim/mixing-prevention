...
<div class="h-auto p-2 m-1 rounded-lg hover:bg-red-50 w-96 md:m-4 md:mt-1 max-h-auto max-w-96 min-w-fit min-h-fit outline outline-1 outline-gray-300">
    ...
    <table class="flex h-auto p-2 text-sm md:flex-col">
        ...
        <tbody  class="p-2">
            ...
            @if(count($array) <= 0)
            <div>
                <td class="p-2 py-10 italic text-center bg-red-100 rounded-lg w-80 outline outline-2 outline-red-500 justify-content-center">No data found</td>
            </div>
    ...
    {{-- 1) TABLE DISPLAY DATA, DELETE BUTTON AND EDIT BUTTON --}}
            @foreach($array  as $data)
                <tr >
                    <td class="flex items-center justify-between p-1 px-2 m-1 bg-gray-100 rounded-lg min-w-auto hover:font-bold outline outline-gray-200 outline-1 hover:outline-2 hover:outline-yellow-300 hover:bg-yellow-50 w-80">{{ $data->{$column} }}
                    <div class="flex flex-row">
                        <a href="{{ url(strtolower($parts ."/").'edit/'.$data->id) }}" >
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-10 hover:text-blue-500" viewBox="0 -0.5 26 26" fill="none">
                        ...
                        <a href="{{ url(strtolower($parts ."/").'delete/'.$data->id) }}">
                           <svg xmlns="http://www.w3.org/2000/svg" class="h-8 mt-1.5 w-9 hover:text-red-600" viewBox="0 0 25 25" fill="none">
