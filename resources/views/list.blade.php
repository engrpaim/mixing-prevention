        ...
        <div class="flex flex-col items-center justify-center ">
            <div class="flex my-10 ">
                <form name="range_form" id="range_form" method="POST" action="{{ url('range-data') }}" >
                    @csrf
                    {{-- Range slector --}}
                  <x-showrange :array="$isDisplayCheckBox"/>
                </form>
            </div>
            <div class="flex ">
            @if ( isset($buttonModels) && !empty($buttonModels))
                <table class="min-w-full my-10 table-auto">
                    <thead >
                        <tr class="bg-gray-100">
                            <th  class="px-4 py-2 text-left border border-gray-300">MODEL&nbsp;NAME</th>
                            <th class="px-4 py-2 text-left border border-gray-300">DIMENSION PROCESS</th>

                        </tr>
                    </thead>
                    <tbody>

                            @foreach ( $buttonModels as $displayResult)
                            <tr>
                                <td  class="px-4 py-2 text-left border border-gray-300">{{ $displayResult }}</td>
                                @if (isset($allDimensionFlow[$displayResult]))




                                    <td  class="px-4 py-2 text-left border border-gray-300 w-fit w-max-screen">

                                        @foreach ( $allDimensionsValue as $keyAllDimensions => $valueAllDimensions )
                                            @php
                                               $removePercent = str_replace("%"," " ,$keyAllDimensions);
                                            @endphp
                                            @if (array_key_exists($displayResult,$valueAllDimensions))
                                                <button
                                                    class="p-2 m-2 text-xs font-bold text-gray-700 bg-gray-100 rounded outline outline-2 outline-gray-500">
                                                    {{ $removePercent.": ".$valueAllDimensions[$displayResult]}}
                                                </button>
                                            @endif
                                        @endforeach
                                    </td>
            ...
            @else
                <div class="flex flex-col p-5 text-red-600 bg-red-100 rounded-lg w-screen2 outline outline-red-500">
